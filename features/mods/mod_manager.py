...
class ModManager:
    ...
    def list_mods_with_status(
        self, shard_name: str = "Master", force_refresh: bool = False
    ) -> List[Dict]:
        """
    ...
    def validate_mod_configuration(
    ...
    def _validate_mod_specific_config(self, workshop_id: str, content: str) -> Dict:
        ...
        if not match:
            result["warnings"].append(
                f"Mod {workshop_id} not found in modoverrides.lua"
            )
            return result
        ...
        if "configuration_options" in config_block:
            if re.search(r"configuration_options\s*=\s*\{\s*\}", config_block):
                result["suggestions"].append(
                    f"Mod {workshop_id}: Empty configuration_options"
                )

        # Check for common typos
        common_typos = [
            "configuraton_options",
            "configration_options",
            "enable",
            "enbaled",
        ]
    ...
    def _validate_against_modinfo(self, workshop_id: str) -> Dict:
        ...
        try:
            ...
            config_options_match = re.search(
    ...
    def fix_common_mod_issues(
        ...
        """
        fix_result = {"fixed": [], "remaining_issues": [], "success": False}
            ...
            if match:
                ...
                if "enabled" not in config_content:
                    ...
                    new_content = content.replace(
                        match.group(0), prefix + new_config + suffix
                    )
                    path.write_text(new_content)
                    return True
    ...
    def update_mod_configuration(
        self, workshop_id: str, config: Dict, shard_name: str = "Master"
    ) -> bool:
    ...
    def import_mod_config(
        self, workshop_id: str, config_data: Dict, shard_name: str = "Master"
    ) -> bool:
        """Import mod configuration."""
        return mod_config_manager.import_mod_config(
            workshop_id, config_data, shard_name
        )
