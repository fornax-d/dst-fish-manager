...
class ModConfigManager:
    ...
    def _parse_single_option(self, option_text: str) -> Optional[Dict]:
        ...
        try:
            ...
            options_match = re.search(
                r"options\s*=\s*\{([^}]*)\}", option_text, re.DOTALL
            )
            if options_match:
    ...
    def _parse_option_choices(self, options_text: str) -> List[Dict]:
        ...
        choice_pattern = (
            r'\{\s*description\s*=\s*"([^"]+)"[^}]*data\s*=\s*([^\s,}]+)[^}]*\}'
        )

        for match in re.finditer(choice_pattern, options_text):
            choice = {"description": match.group(1), "data": match.group(2)}
    ...
    def get_current_mod_config(
        ...
        try:
            modoverrides_path = (
                self.dst_dir / self.cluster_name / shard_name / "modoverrides.lua"
            )
            ...
            config_options_match = re.search(
                r"configuration_options\s*=\s*\{([^}]*)\}", config_block, re.DOTALL
            )
    ...
    def update_mod_config(
        ...
        try:
            ...
            if re.search(mod_pattern, content, re.DOTALL):
                ...
                new_content = re.sub(
                    mod_pattern, new_mod_entry, content, flags=re.DOTALL
                )
            else:
                # Add new mod
                new_mod_entry = self._create_mod_entry(workshop_id, config_str)
